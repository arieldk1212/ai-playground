...
#define PRINT_RSMI_ERR(RET)                                              \
  {                                                                      \
    if (RET != RSMI_STATUS_SUCCESS) {                                    \
      std::cout << "[ERROR] RSMI call returned " << (RET) << " at line " \
                << __LINE__ << "\n";                                     \
...
#define CHK_AND_PRINT_RSMI_ERR_RET(RET) {PRINT_RSMI_ERR(RET) CHK_RSMI_RET(RET)}
...
void print_function_header_with_rsmi_ret(rsmi_status_t myReturn,
                                         std::string header = "") {
  std::cout << "\t** ";
  if (!header.empty()) {
    std::cout << header << ": ";
...
static const char* power_profile_string(
...
static const std::string compute_partition_string(
    rsmi_compute_partition_type_t partition) {
  switch (partition) {
...
static const std::string memory_partition_string(
    rsmi_memory_partition_type_t partition) {
  switch (partition) {
...
static const char* perf_level_string(rsmi_dev_perf_level_t perf_lvl) {
  switch (perf_lvl) {
...
static const std::string clock_type_string(rsmi_clk_type_t clk) {
  switch (clk) {
...
static rsmi_status_t test_power_profile(uint32_t dv_ind) {
  ...
  CHK_RSMI_RET(ret)
  std::cout << "Performance Level is now " << perf_level_string(pfl) << "\n";
...
static rsmi_status_t test_power_cap(uint32_t dv_ind) {
  ...
  std::cout << "Power Cap Range: " << max << " uW to " << min << " uW" << "\n";
  new_cap = (max + min) / 2;
...
static rsmi_status_t test_set_freq(uint32_t dv_ind) {
  ...
  for (uint32_t clk = (uint32_t)RSMI_CLK_TYPE_FIRST; clk <= RSMI_CLK_TYPE_LAST;
       ++clk) {
    ...
    std::cout << "Setting frequency mask for clock " << rsmi_clk << " to 0b"
              << freq_bm_str << " ..." << "\n";
...
static rsmi_status_t test_set_compute_partitioning(uint32_t dv_ind) {
  ...
  ret =
      rsmi_dev_compute_partition_get(dv_ind, originalComputePartition, kLength);
  ...
  std::cout << "Original Memory Partition: "
            << (((originalMemoryPartition == nullptr) ||
                 ((originalMemoryPartition != nullptr) &&
                  (originalMemoryPartition[0] == '\0')))
                    ? "UNKNOWN"
                    : originalMemoryPartition)
            << "\n\n";

  for (int newMemPartition = RSMI_MEMORY_PARTITION_NPS1;
       newMemPartition <= RSMI_MEMORY_PARTITION_NPS8; newMemPartition++) {
...
template <typename T>
constexpr float convert_mw_to_w(T mw) {
  ...
  if (status_code == rsmi_status_t::RSMI_STATUS_SUCCESS) {
    if constexpr (std::is_array_v<T>) {
      ...
      for (const auto& el : metric) {
        str_values +=
            "\t\t  [" + std::to_string(idx) + "]: " + std::to_string(el) + "\n";
        ++idx;
      }
      return str_values;
